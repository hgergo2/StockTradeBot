...
class LiveChart:
    ...
    def add_subplot_data(self, data: [], graph_type: str, color='blue', show_legend=False):
        ...
        if graph_type == 'line':
            self.chart.add_trace(go.scatter.Line(x=data.index, y=data, showlegend=show_legend, line=dict(color=color)),
                                 row=2, col=1)
        elif graph_type == 'bar':
            self.chart.add_trace(go.scatter.Bar(x=data.index, y=data, showlegend=show_legend, marker_color=color),
                                 row=2, col=1)
